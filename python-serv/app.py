...
def auth():
	...
	validator = Validator().set_rules([
		Rule('phone', data_type.STRING),
		Rule('password', data_type.STRING)
	])
	...
	for auth in auths['data'].items():
		if content['phone'] == auth[1]['phone'] and \
		   content['password'] == auth[1]['password']:
			match = {
				'name': auth[1]['name'],
				'phone': auth[1]['phone'],
				'role': auth[1]['role'],
	...
	return jsonify(
		{
			'access_token': jwt.encode(match, password, algorithm='HS256').decode('utf-8')
		}
	), http.HTTP_OK
